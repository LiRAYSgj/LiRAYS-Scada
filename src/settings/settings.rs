...
/// - `default` is used only when neither env nor YAML provides a value.
#[derive(Debug, Clone)]
...
/// Error type returned by the settings loader when values cannot be read or parsed.
#[derive(Debug)]
...
impl std::fmt::Display for SettingsError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            SettingsError::InvalidEnvValue { env_var, reason } => {
                write!(
                    f,
            ...
            SettingsError::InvalidSettingValue {
                section,
                key,
                reason,
            } => write!(
...
impl Settings {
    ...
    pub fn resolve<'a, T>(
        &'a self,
        spec: &SettingSpec<'a, T>,
    ) -> Result<(T, ValueSource), SettingsError>
    where
        T: Clone + DeserializeOwned + EnvParse,
    {
        ...
        if let Some(raw) = env::var_os(spec.env_var) {
            ...
            match T::parse_env(&raw_str) {
                ...
                Err(err) => {
                    return Err(SettingsError::InvalidEnvValue {
        ...
        if let Some(val) = self.lookup_yaml_value(spec.section, spec.key) {
            match serde_yaml::from_value::<T>(val.clone()) {
                ...
                Err(err) => {
                    return Err(SettingsError::InvalidSettingValue {
...
"#
    ...
    #[test]
    fn invalid_env_value_fails_resolution() {
        let file_path = temp_path("settings_invalid_env");
        let mut file = File::create(&file_path).unwrap();
        write!(
            file,
            r#"
...
"#
        ...
        let loader = Settings::from_optional_file(Some(&file_path)).unwrap();
        unsafe {
            std::env::set_var("TEST_PORT_INVALID", "not-a-number");
        }

        let spec = SettingSpec {
            section: "server",
            key: "port",
            env_var: "TEST_PORT_INVALID",
            default: 8000u16,
        };

        let err = loader.resolve(&spec).unwrap_err();
        assert!(matches!(err, SettingsError::InvalidEnvValue { .. }));
    ...
    #[test]
    fn invalid_yaml_value_fails_resolution() {
        let file_path = temp_path("settings_invalid_yaml_value");
        let mut file = File::create(&file_path).unwrap();
        write!(
            file,
            r#"
...
"#
        ...
        let spec = SettingSpec {
            section: "server",
            key: "port",
            env_var: "TEST_PORT_INVALID_YAML",
            default: 8000u16,
        };

        let err = loader.resolve(&spec).unwrap_err();
        assert!(matches!(err, SettingsError::InvalidSettingValue { .. }));
    }
}
