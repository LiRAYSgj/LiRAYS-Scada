...
use lirays_scada_proto::namespace::v1::{
    DelCommand, Event, FolderChanged, FolderInfo, ItemMeta, TreeChanged, VarInfo, VarMetaChanged,
    event::Ev,
};

use super::utils::{get_parent_and_name, normalize_path};
...
pub fn extract_add_event(
    ...
    let new_folders = new_folders_imeta
        .iter()
        .map(|i_meta| FolderInfo {
            id: normalize_path(&format!("{}/{}", folder_id, i_meta.name)),
            name: i_meta.name.to_string(),
        })
        .collect();
    let new_variables = new_variables_imeta
        .iter()
        .map(|i_meta| {
...
pub fn extract_del_event(del_cmd: &DelCommand) -> Result<Event, String> {
    ...
    let event = Event {
        ev: Some(Ev::TreeChangedEv(TreeChanged {
            folder_changed_event: removed_data
                .iter()
                .map(|(parent, removed)| FolderChanged {
...
/// Build a TreeChanged event for an edited variable with id `var_id`.
pub fn extract_edit_event(
    var_id: &str,
    unit: Option<String>,
    min: Option<f64>,
    max: Option<f64>,
    ...
    let var_meta_changed = vec![VarMetaChanged {
    ...
    Ok(Event {
        ev: Some(Ev::TreeChangedEv(TreeChanged {
            ...
            var_meta_changed_event: var_meta_changed,
        })),
    })
}
...
#[cfg(test)]
mod tests {
    use lirays_scada_proto::namespace::v1::DelCommand;

    use super::*;

    #[test]
    fn add_event_builds_ids() {
        let folders = vec![ItemMeta {
            name: "f1".into(),
            i_type: 1,
            var_d_type: None,
            unit: None,
    ...
    fn del_event_groups_and_normalizes() {
        let dc = DelCommand {
            cmd_id: "c1".into(),
            item_ids: vec!["/Root/f1".into(), "/Root/f1/v1".into()],
        };
