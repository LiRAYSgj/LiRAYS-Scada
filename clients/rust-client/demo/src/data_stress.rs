...
const NUM_FOLDERS: usize = 500;
const VARS_PER_FOLDER: usize = 200;
const NUM_CLIENTS: usize = 50;
const PCT_TOUCH: usize = 30; // percentage of vars each client sets/gets per tick
            ...
            while start.elapsed() < Duration::from_secs(RUN_SECS) {
                let (subset, values, pause_ms) = {
                    ...
                    let subset: Vec<String> = ids
                        ...
                        .cloned()
                        .collect();
                    let values: Vec<f64> = subset.iter().map(|_| rng.gen_range(-1000.0..1000.0)).collect();
