# SCADA Frontend (Static SPA)

Interactive SCADA-style frontend built with SvelteKit + TypeScript.

This frontend is built as a client-only SvelteKit SPA and typically served either by the backend stack or a static host.
...
- `npm run start` - preview the built static app
...
- Realtime flow uses websocket subscriptions plus on-demand `GET` refreshes for tracked tags.
...
- Edit/Play modes:
  - Edit: drag Variables from tree to canvas and modify graph
