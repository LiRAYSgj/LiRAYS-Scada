...
import {
  ...
  type GetResponse,
  type ListResponse,
} from "@lirays/scada-proto";
import {
  VarIdValue as VarIdValueMessage,
  type VarIdValue,
} from "@lirays/scada-proto";
import { Event, type TreeChanged } from "@lirays/scada-proto";
import {
  EventType,
  ItemType,
  OperationStatus,
...
export class TagStreamClient {
  ...
  stop(): void {
    const idsToUnsubscribe = [...this.desiredIds];
    if (
      this.socket &&
      this.socket.readyState === WebSocket.OPEN &&
      idsToUnsubscribe.length > 0
    ) {
      // Best effort: notify backend to drop current VAR_VALUES subscriptions before close.
      try {
        const { command } = createUnsubscribeCommand(idsToUnsubscribe, [
          EventType.EVENT_TYPE_VAR_VALUES,
        ]);
        this.send(command);
      } catch {
        // Ignore unsubscribe framing errors during shutdown.
      }
    }
  ...
  async getValues(
    varIds: string[],
    endpoint?: string,
  ): Promise<Record<string, TagScalarValue>> {
    const ids = [...new Set(varIds.filter(Boolean))];
    if (ids.length === 0) {
      return {};
    }
    ...
    return new Promise<Record<string, TagScalarValue>>((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        this.pendingByCmdId.delete(cmdId);
        snackbarStore.error(TIMEOUT_USER_MESSAGE);
      ...
      this.pendingByCmdId.set(cmdId, {
        type: "get",
        resolve: (value) => {
          const payload = value as GetResponse;
          const parsed: Record<string, TagScalarValue> = {};
          for (let index = 0; index < ids.length; index += 1) {
            if (index >= payload.varValues.length) {
              break;
            }
            const varId = ids[index];
            const nextValue = fromBackendValue(payload.varValues[index]?.value);
            if (nextValue !== undefined) {
              parsed[varId] = nextValue;
            }
          }
          resolve(parsed);
        },
  ...
  private send(message: Command): boolean {
    if (!this.socket || this.socket.readyState !== WebSocket.OPEN) {
      return false;
    }
  ...
  private rejectPendingBeforeSend(
    cmdId: string,
    timeoutId: ReturnType<typeof setTimeout>,
    reject: (reason?: unknown) => void,
    message: string,
  ...
  private async subscribeIds(ids: string[]): Promise<void> {
    ...
    void this.requestLatestValues(ids);
  }

  private async requestLatestValues(ids: string[]): Promise<void> {
    if (ids.length === 0) return;
    await this.ensureConnected();
    const { cmdId, command } = createGetCommand(ids);
    this.pendingByCmdId.set(cmdId, {
      type: "get",
      resolve: (v) => {
        this.applyGetValues(ids, v as GetResponse);
      },
      reject: () => {
        snackbarStore.error("Get latest values request failed");
      },
      timeoutId: setTimeout(() => {
        this.pendingByCmdId.delete(cmdId);
      }, COMMAND_TIMEOUT_MS),
  ...
  private applyGetValues(varIds: string[], payload: GetResponse): void {
    if (varIds.length === 0 || payload.varValues.length === 0) {
      return;
    }
    this.values.update((current) => {
      const next = { ...current };
      for (let index = 0; index < varIds.length; index += 1) {
        if (index >= payload.varValues.length) {
          break;
        }
        const varId = varIds[index];
        if (!this.desiredIds.has(varId)) {
          continue;
        }
        const parsed = fromBackendValue(payload.varValues[index]?.value);
        if (parsed === undefined) {
          delete next[varId];
        } else {
          next[varId] = parsed;
  ...
  private handleMessage(rawData: unknown): void {
    ...
    try {
      ...
      if (hasEnvelopeType) {
        ...
        const status = payload.status ?? OperationStatus.UNRECOGNIZED;
        ...
        if (isTerminalSuccess) {
          ...
          if (pending) {
            ...
            } else if (pending.type === "get" && payload.get != null) {
              pending.resolve(payload.get);
            } else {
              pending.resolve();
            }
          }
