...
import { Event } from "@lirays/scada-proto";
import { EventType, ItemType, OperationStatus } from "@lirays/scada-proto";
...
describe("TagStreamClient", () => {
  ...
  it("hydrates initial values from GetResponse after subscribe", async () => {
    const client = new TagStreamClient();
    client.start("ws://localhost:8787");
    const ws = FakeWebSocket.instances[0];
    ws.readyState = FakeWebSocket.OPEN;
    ...
    client.setTrackedIds(["tag-a", "tag-b"]);
    await vi.waitFor(() => {
      expect(ws.sent.some((msg) => Command.decode(msg).get !== undefined)).toBe(
        true,
      );
    });

    const getMsg = ws.sent.find((msg) => Command.decode(msg).get !== undefined);
    expect(getMsg).toBeDefined();
    const getReq = Command.decode(getMsg!).get!;
    ...
    expect(get(client.values)).toEqual({ "tag-a": 12, "tag-b": "ready" });
  });
  ...
  it("rejects command immediately when send cannot write to socket", async () => {
    const client = new TagStreamClient();
    client.start("ws://localhost:8787");
    const ws = FakeWebSocket.instances[0];
    ws.readyState = FakeWebSocket.OPEN;
    ws.emit("open");
    (client as unknown as { send: (message: Command) => boolean }).send = () =>
      false;

    await expect(client.listChildren(undefined)).rejects.toThrow(
