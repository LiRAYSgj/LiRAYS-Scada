...
describe('namespace-yaml', () => {
	...
	describe('composeNodeName', () => {
		it('returns name when no range', () => {
			expect(composeNodeName({ name: 'Power', rangeEnd: '', rangeStart: '', rangeStep: '' } as NamespaceNode)).toBe('Power');
		});
	...
	describe('setKindFromChildren', () => {
		it('sets kind to folder and dataType to null when node has children', () => {
			const node: NamespaceNode = {
				...
				rangeEnd: '',
				rangeStep: '',
				children: [{ id: '2', name: 'C', kind: 'variable', dataType: 'Float', rangeStart: '', rangeEnd: '', rangeStep: '', children: [] }]
			};
	...
	describe('parseYamlLike', () => {
		...
		it('parses range notation in variable name', () => {
			const roots = parseYamlLike('Var_[0:100]: Float', {}, ALLOWED_TYPES, nextId);
		...
		it('accepts plain name as folder when skipLeafTypeValidation', () => {
			const roots = parseYamlLike('typing', { skipLeafTypeValidation: true }, ALLOWED_TYPES, nextId);
			expect(roots).toHaveLength(1);
		...
		it('throws on invalid type', () => {
			expect(() => parseYamlLike('X: UnknownType', {}, ALLOWED_TYPES, nextId)).toThrow(/Invalid YAML-like line|unknown type/);
		});

		it('throws on variable with indented children', () => {
			const yaml = 'Motor_: Float\n  Power: Float';
			expect(() => parseYamlLike(yaml, {}, ALLOWED_TYPES, nextId)).toThrow(/declares a data type.*indented content/);
		});

		it('throws on leaf without type when strict', () => {
			const yaml = 'Folder_:';
			expect(() => parseYamlLike(yaml, {}, ALLOWED_TYPES, nextId)).toThrow(/leaf without type|must declare a data type/);
		});
	...
	describe('serializeYamlLike', () => {
		it('serializes variable', () => {
			const nodes: NamespaceNode[] = [
				{ id: '1', name: 'Power', kind: 'variable', dataType: 'Float', rangeStart: '', rangeEnd: '', rangeStep: '', children: [] }
			];
					...
					children: [
						{
							...
							rangeStep: '',
							children: [
								{ id: '3', name: 'Power', kind: 'variable', dataType: 'Float', rangeStart: '', rangeEnd: '', rangeStep: '', children: [] }
							]
						}
					]
				}
	...
	describe('validateNamespaceAst', () => {
			...
			const nodes: NamespaceNode[] = [
				{
					...
					rangeEnd: '',
					rangeStep: '',
					children: [{ id: '2', name: 'X', kind: 'variable', dataType: 'Float', rangeStart: '', rangeEnd: '', rangeStep: '', children: [] }]
				}
			];
		...
		it('throws for leaf folder without type', () => {
			const nodes: NamespaceNode[] = [
				{ id: '1', name: 'X', kind: 'folder', dataType: null, rangeStart: '', rangeEnd: '', rangeStep: '', children: [] }
			];
			expect(() => validateNamespaceAst(nodes, ALLOWED_TYPES)).toThrow(/leaf without type|must declare a data type/);
		});
	});
			...
			const nodes: NamespaceNode[] = [
				{
					...
					rangeEnd: '',
					rangeStep: '',
					children: [{ id: '2', name: 'X', kind: 'variable', dataType: 'Integer', rangeStart: '', rangeEnd: '', rangeStep: '', children: [] }]
				}
			];
			normalizeLeafFoldersToVariables(nodes, ALLOWED_TYPES);
	...
	describe('astToNamespaceJson', () => {
		it('produces nested object with type strings at leaves', () => {
			...
			expect((json['Area_[0:20]'] as Record<string, unknown>)['Line_[:4]']).toEqual({ Power: 'Float' });
		});
	});
	...
	describe('mergeTrailingBlankLinesAfterAutoFill', () => {
		it('pads after with blank lines when before has trailing blanks', () => {
			const before = 'a:\n  b: Float\n\n';
			const after = 'a:\n  b: Float';
			expect(mergeTrailingBlankLinesAfterAutoFill(before, after, 3)).toBe('a:\n  b: Float\n\n');
		});

		it('returns after when cursor not on blank line and same line count', () => {
			const before = 'a:\n  b: Float';
			const after = 'a:\n  b: Float';
			expect(mergeTrailingBlankLinesAfterAutoFill(before, after, 1)).toBe(after);
		});
	});
