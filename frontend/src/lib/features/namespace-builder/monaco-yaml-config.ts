...
export function getYamlLiteTokenizer(): import('monaco-editor').languages.IMonarchLanguage {
	return {
		tokenizer: {
			root: [
				[/^(\s*)((?:[^:\[]|\[[^\]]*\])+)(:\s*)(\S+)(\s*)$/, ['', 'variable', 'delimiter', 'type', '']],
...
export function getMonacoThemeId(colorMode: 'light' | 'dark'): string {
	return colorMode === 'dark' ? UNS_YAML_THEME_DARK_ID : UNS_YAML_THEME_LIGHT_ID;
}

export function getYamlCompletionProvider(monaco: typeof import('monaco-editor'), allowedDataTypes: string[]) {
	return {
		triggerCharacters: [':', ' '],
		provideCompletionItems(model: import('monaco-editor').editor.ITextModel, position: import('monaco-editor').Position) {
			...
			const currentIndent = Math.floor(lineIndentStr.replace(/\t/g, '  ').length / 2);
			for (let ln = position.lineNumber + 1; ln <= model.getLineCount(); ln += 1) {
				const nextLine = model.getLineContent(ln);
				if (!nextLine.trim()) continue;
				const nextIndentStr = (nextLine.match(/^[ \t]*/)?.[0] ?? '');
				const nextIndent = Math.floor(nextIndentStr.replace(/\t/g, '  ').length / 2);
...
export async function removeEditorContextMenuItems(monaco: typeof import('monaco-editor')): Promise<void> {
	try {
		// @ts-expect-error Monaco internal ESM path has no public types
		const actions = await import('monaco-editor/esm/vs/platform/actions/common/actions.js');
		const { MenuRegistry, MenuId } = actions as { MenuRegistry?: { _menuItems?: Map<unknown, unknown> }; MenuId?: { EditorContext?: unknown } };
		...
		while (node && node !== end) {
			const current = node as { next: unknown; element?: { command?: { id?: string } } };
			const nextNode = current.next;
			if (current.element?.command?.id && removableIds.includes(current.element.command.id)) {
				list._remove(node);
			}
			node = nextNode;
		}
