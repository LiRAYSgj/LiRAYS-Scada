<script lang="ts">
	...
	const skeletonRows = Array.from({ length: 14 }, (_, index) => `skeleton-${index}`);
	const ROW_HEIGHT = 32;
	const OVERSCAN = 10;
	...
	let addName = $state('');
	let addKind = $state<'Folder' | 'Variable'>('Variable');
	let addDataType = $state<BackendVarDataType>('Text');
	let addDefaultValue = $state<string | number>('');
	let addError = $state('');
	let addSubmitting = $state(false);
	let addParentId = $state<string | null | undefined>(undefined);
	const isConnected = $derived(websocketStatus === WebSocketConnectionStatus.CONNECTED);

	const totalRows = $derived($visibleRows.length);
	const startIndex = $derived(Math.max(0, Math.floor(scrollTop / ROW_HEIGHT) - OVERSCAN));
	const visibleCount = $derived(Math.max(1, Math.ceil(viewportHeight / ROW_HEIGHT) + OVERSCAN * 2));
	...
	onMount(() => {
		...
		window.addEventListener('tree:open-add-dialog', openAddDialogHandler as EventListener);

		return () => {
			observer.disconnect();
			window.removeEventListener('tree:refresh', refreshHandler as EventListener);
			window.removeEventListener('tree:open-add-dialog', openAddDialogHandler as EventListener);
		};
	});
	...
	function resolveDefaultInputType(dataType: BackendVarDataType): 'text' | 'number' {
		return dataType === 'Integer' || dataType === 'Float' ? 'number' : 'text';
	}

	function resolveDefaultInputStep(dataType: BackendVarDataType): string | undefined {
		if (dataType === 'Integer') {
			return '1';
		}
		if (dataType === 'Float') {
	...
	function resolveDefaultInputMin(dataType: BackendVarDataType): string | undefined {
	...
	function resolveParentIdForCreate(): string | null {
		...
		const snapshot = get(treeState);
		const selected = snapshot.selectedId ? snapshot.nodes[snapshot.selectedId] : null;
	...
	async function submitAddDialog(event: SubmitEvent): Promise<void> {
		...
		addSubmitting = true;
		try {
			const itemType: BackendItemType = addKind;
			const varType: BackendVarDataType | null = itemType === 'Variable' ? addDataType : null;
			const defaultValue =
				itemType === 'Variable' ? parseDefaultValue(addDefaultValue, addDataType) : undefined;
			await onCreateItem({
				parentId,
				name: addName.trim(),
				itemType,
			...
			addError = error instanceof Error ? error.message : 'Failed to create item';
		} finally {
			addSubmitting = false;
		}
	}
...
>
	...
	>
		{#if !$treeState.hasInitialized || $treeState.rootLoading}
			<div class="space-y-2 p-2">
				{#each skeletonRows as rowKey (rowKey)}
					<div class="grid h-8 grid-cols-[1fr_90px_90px] items-center gap-2 rounded px-2">
		...
		{:else}
			<div role="tree" tabindex="0" class="outline-none" onkeydown={handleTreeKeyDown}>
	...
	>
		<div class="flex items-center justify-start">
			...
			{:else if websocketStatus === WebSocketConnectionStatus.CONNECTED}
				...
				/>
			{:else}
				<Circle class="h-3.5 w-3.5 fill-red-500 text-red-500" aria-label="WebSocket disconnected" />
			{/if}
		</div>
	</footer>
</section>
...
>
	<form class="flex h-full flex-col p-4" onsubmit={submitAddDialog}>
		...
		<div class="space-y-4">
			...
			<div class="space-y-1">
				<label class="text-xs text-(--text-muted)" for="add-kind">Node Type</label>
				<select
					id="add-kind"
					class="w-full rounded border border-black/15 bg-(--bg-muted) px-2 py-1.5 text-sm outline-none ring-0 focus:border-blue-500 dark:border-white/10"
					bind:value={addKind}
			...
			<div class={`space-y-1 ${addKind !== 'Variable' ? 'invisible' : ''}`}>
				<label class="text-xs text-(--text-muted)" for="add-dataType">Data Type</label>
				<select
					id="add-dataType"
					class="w-full rounded border border-black/15 bg-(--bg-muted) px-2 py-1.5 text-sm outline-none ring-0 focus:border-blue-500 dark:border-white/10"
					bind:value={addDataType}
					disabled={addKind !== 'Variable'}
				>
			...
			<div class={`space-y-1 ${addKind !== 'Variable' ? 'invisible' : ''}`}>
				<label class="text-xs text-(--text-muted)" for="add-defaultValue">Default Value</label>
				{#if addDataType === 'Boolean'}
					<select
						id="add-defaultValue"
						class="w-full rounded border border-black/15 bg-(--bg-muted) px-2 py-1.5 text-sm outline-none ring-0 focus:border-blue-500 dark:border-white/10"
				...
				{:else}
					<input
						...
						placeholder={addDataType === 'Text' ? 'Optional text value' : 'Optional numeric value'}
					/>
				{/if}
			</div>
		</div>
		...
		<div class="mt-auto flex justify-end gap-2 border-t border-black/10 pt-4 dark:border-white/10">
			<button
				type="button"
