...
@mock_router.post("/resource/", response_model=MockResource)
async def create_resource(payload: MockResource, db: SQLiteEng = Depends(get_db_eng)):
    try:
...
async def list_resources(db: SQLiteEng = Depends(get_db_eng)):
    try:
